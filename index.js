...
function filterFundedOnly() {
    ...
    const funded = GAMES_JSON.filter((game) => { return game.pledged >= game.goal});
...
function description() {
    if (unfunded.length === 1) {
        return `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. We have ${unfunded.length} game that needs your help!`;
    } else {
        return `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. We have ${unfunded.length} games that need your help!`;
    }
}
